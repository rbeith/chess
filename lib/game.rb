...
class Game
  ...
  def same_color?(player, start_piece)
    true if player.color == start_piece.color
  end
  ...
  def verify_input(player, start_piece, end_space, start_space)
    ...
    verified = true
    verified = false if same_color?(player, start_piece) == false ||
                        @game_board.forbidden?(start_space, end_space, start_piece) == true
