...
module ChessBoard
  ...
  def place_pieces
    ...
    @piece[0][3] = WhiteQueen.new(position: [0, 3])
		@white_king = WhiteKing.new
    @piece[0][4] = @white_king
    ...
    @piece[1].map! { |_i| i = WhitePawn.new(position: [1, nil]) }
    @piece[6].map! { |_i| i = BlackPawn.new(position: [6, nil]) }
    ...
    @piece[7][3] = BlackQueen.new(position: [7, 3])
		@black_king = BlackKing.new(position: [7, 4])
    @piece[7][4] = @black_king
